// Check Wheter List Is Sorter(Ascendig order).
...
/*
    Algorithm
        Start
            ...
            and 
            Check that elements which are sorted in Ascending order or not. 
        Stop
*/
...
BOOL CheckSorted(PNODE first, int no)
{
    PNODE temp = NULL;
...
int main()
{
    ...
    Display(head);
    iRet = CheckSorted(head,iValue);

    if(iRet == 1)
    {
        printf("Yes, The LL is Sortern in Ascending Order.");
    }
    else
    {
        printf("No, The LL is not Sortern in Ascending Order.");
    }
